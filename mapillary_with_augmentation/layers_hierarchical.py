...
def make_variable_with_weight_decay(name, shape, stddev=0.01, wd=0.0005):
    ...
    var = make_variable(name, shape, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), regularizer=regularizer)
    return var
...
def make_graphcnn_layer(V, A, no_filters, name=None):
    ...
    V = tf.cast(V, tf.float32)
    #no_filters = tf.cast(no_filters, tf.float32)
    with tf.variable_scope(name, default_name='Graph-CNN') as scope:
        print("+++++")
        print(A_cast.get_shape())
        print(V.get_shape())
        print(no_filters)
        print(GraphCNNGlobal.GRAPHCNN_INIT_FACTOR)
        no_A = A_cast.get_shape()[2].value
        ...
        A_shape = tf.shape(A)
        B = A_shape[1]*no_A
        C = tf.stack([-1, A_shape[1]*no_A, A_shape[1]])
        
        A_shape = tf.shape(A_cast)
        A_reshape = tf.reshape(A_cast, tf.stack([-1, A_shape[1]*no_A, A_shape[1]]))
...
def make_graph_embed_pooling(V, A, no_vertices=1, mask=None, name=None):
    ...
    with tf.variable_scope(name, default_name='GraphEmbedPooling') as scope:
        ...
        result_A = tf.reshape(result_A, (tf.shape(A)[0], no_vertices, no_A, no_vertices))
...
class GraphConvolution(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight("kernel", (shape, self.output_dim), initializer=init)
        self.bias = self.add_weight("bias", (self.output_dim,), initializer="zeros", trainable=True)
...
class GraphPooling(tf.keras.layers.Layer):
    def __init__(self, pool_ratio=0.5, **kwargs):
    ...
    def call(self, inputs):
        x, adj = inputs
        x = tf.matmul(adj, x)
        x = tf.nn.max_pool1d(x, self.pool_ratio)
        return x, adj

class GraphUnPooling(tf.keras.layers.Layer):
