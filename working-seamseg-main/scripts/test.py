...
parser.add_argument("--meta", type=str,
                    help="Path to metadata file of training dataset")
parser.add_argument("--score_threshold", type=float,
                    default=0.5, help="Detection confidence threshold")
parser.add_argument("--iou_threshold", type=float, default=0.5,
                    help="Panoptic disambiguation IoU threshold")
parser.add_argument("--min_area", type=float, default=4096,
                    help="Minimum pixel area for stuff predictions")
parser.add_argument("--raw", action="store_true",
                    help="Save raw predictions instead of rendered images")
...
def make_dataloader(args, config, rank, world_size):
    ...
    test_sampler = DistributedARBatchSampler(
        test_db, 1, world_size, rank, False)
...
def make_model(config, num_thing, num_stuff):
    ...
    classes = {"total": num_thing + num_stuff,
               "stuff": num_stuff, "thing": num_thing}
    ...
    body_params = body_config.getstruct(
        "body_params") if body_config.get("body_params") else {}
    ...
    rpn_algo = RPNAlgoFPN(
        ...
        rpn_config.getint("anchor_scale"), rpn_config.getstruct(
            "anchor_ratios"),
        fpn_config.getstruct("out_strides"), rpn_config.getint("fpn_min_level"), rpn_config.getint("fpn_levels"))
    rpn_head = RPNHead(
        fpn_config.getint("out_channels"), len(
            rpn_config.getstruct("anchor_ratios")), 1,
    ...
    roi_algo = InstanceSegAlgoFPN(
        ...
        roi_config.getstruct("bbx_reg_weights"), roi_config.getint(
            "fpn_canonical_scale"),
        roi_config.getint("fpn_canonical_level"), roi_size, roi_config.getint(
            "fpn_min_level"),
        roi_config.getint("fpn_levels"), lbl_roi_size, roi_config.getboolean("void_is_background"))
    roi_head = FPNMaskHead(fpn_config.getint(
        "out_channels"), classes, roi_size, norm_act=norm_act_dynamic)
    ...
    sem_head = FPNSemanticHeadDeeplab(fpn_config.getint("out_channels"),
                                      ...
                                      classes["total"],
                                      pooling_size=sem_config.getstruct(
                                          "pooling_size"),
...
def test(model, dataloader, result_df, rank, **varargs):
    model.eval()
    ...
    for it, batch in enumerate(dataloader):
        with torch.no_grad():
            # Extract data
            img = batch["img"].cuda(
                device=varargs["device"], non_blocking=True)
            ...
            for i, (sem_pred, bbx_pred, cls_pred, obj_pred, msk_pred) in enumerate(zip(
                ...
                result_df = save_function(
                    raw_pred, panoptic_pred, img_info, r=result_df)
            ...
            data_time = time.time()
    result_df.to_csv(f'results_rank{rank}.csv', index=False)
...
def save_prediction_image(_, panoptic_pred, img_info, out_dir, colors, num_stuff, r):
    ...
    #     msk, mode="outer", background=0).astype(np.uint8) * 255
    ...
    img_path = '/'.join(parts[4:])
    features = panoptic_pred[1].numpy().tolist()
    confidence = panoptic_pred[2].numpy().tolist()
    r = r.append(
        {'path': img_path, 'features': features, 'confidence': confidence}, ignore_index=True)
    return r
...
def main(args):
    ...
    result_df = pd.DataFrame(columns=['path', 'features', 'confidence'])
    ...
    torch.backends.cudnn.benchmark = config["general"].getboolean(
        "cudnn_benchmark")
    model = DistributedDataParallel(model.cuda(device), device_ids=[
    ...
    test(model, test_dataloader, result_df, rank, device=device, summary=None,
